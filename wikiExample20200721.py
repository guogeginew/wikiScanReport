...
def export_content(res_soup_api,class_name,sheet_name):
    table_api_value = BeautifulSoup(str(res_soup_api.find_all('table',class_=class_name)[0]),'lxml')
    if sheet_name == u"移交任务（已完成）":
        table_api_value = BeautifulSoup(str(res_soup_api.find_all('table',class_=class_name)[1]),'lxml')
    #生成标题
